...
jest.mock('../app/services/decision', () => ({
  generateDecision: jest.fn().mockResolvedValue('Ini adalah respons test dari MirrAI.'),
}))
...
describe('Chat API', () => {
  ...
  beforeAll(async () => {
    ...
    const loginRes = await request(app).post('/api/auth/login').send({ name, password: 'testpass123' })
